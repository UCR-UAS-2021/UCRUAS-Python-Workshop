...
# When you are finished with each problem, run it to make sure the output matches what you expect. Then, re-comment that problem 
...
# if this is in the Solutions folder, the string should be '../dog.jpg'
...
blur = cv2.GaussianBlur(gray_img, (3, 3), 100)
...
for c in contours:
    ...
    if cv2.contourArea(c) <= 150 or cv2.contourArea(c) >= 250:
        continue
